...
2. **特定のプロジェクトだけ常時**: そのプロジェクトの指示ファイル（`CLAUDE.md` / `AGENTS.md` 等）に下の1行を書くと、そのフォルダでの作業だけ言わなくても常時効きます。ブログのrepoにだけ効かせたい、といった使い方はこれです
3. **全部に常時**: 各ホストの共通指示（`~/.claude/CLAUDE.md` 等）に同じ1行を足すと、そのAIの全ての文章仕事に効きます
